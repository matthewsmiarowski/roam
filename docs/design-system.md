...
- Placeholder text sets the tone: *"Describe your ride — e.g., 60km hilly loop from Girona"*
...
- Displayed as a compact stat grid: distance, elevation gain, elevation loss, estimated time.
- Each stat: large `stat-value` number + small `stat-unit` label below.
- Use `font-variant-numeric: tabular-nums` so numbers are stable.
- Bold dividers or card boundaries between stats.
...
- Progress indication over the map — pulsing accent-colored bar or spinner.
- Short status text: "Generating route...", "Routing through waypoints..."
...
- Icons are always paired with text labels in v0 — no icon-only buttons except map controls.
